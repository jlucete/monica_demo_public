...
function FourierTransform(bufferSize, sampleRate) {
  ...
  this.spectrum   = new Float64Array(bufferSize/2+1);
  this.real       = new Float64Array(bufferSize);
  this.imag       = new Float64Array(bufferSize);

  this.coefficitent_table = new Float64Array(bufferSize);
  ...
  this.calculateSpectrum = function() {
    ...
    for (var i = 0, N = bufferSize/2+1; i < N; i++) {
      rval = real[i];
      ival = imag[i];
...
function FFT(bufferSize, sampleRate) {
  ...
  this.coefficient_table = new Float64Array(bufferSize);
  ...
  //Make Hann window coefficient table
  for (i = 0; i< bufferSize; i++) {
    this.coefficient_table[i] = WindowFunction.Hann(bufferSize, i);
  }

  //Debugging purpose
  look_reverseTable = this.reverseTable.slice();
...
FFT.prototype.forward = function(buffer) {
  ...
  var bufferSize      = this.bufferSize,
      ...
      coefficient_table = this.coefficient_table;
      after_window_buffer = new Float64Array(bufferSize);
  ...
  for (i = 0; i < bufferSize; i++) {
    after_window_buffer[i] = buffer[i]*coefficient_table[i];
  }



  for (i = 0; i < bufferSize; i++) {
    //real[i] = buffer[reverseTable[i]]; // dsp.js
    real[i] = 2*after_window_buffer[reverseTable[i]]; // ckfft
    imag[i] = 0;
  }
